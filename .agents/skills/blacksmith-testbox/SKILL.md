...
1. Warm up immediately when you receive a coding task:
   `blacksmith testbox warmup blacksmith-testbox.yml` → save the ID
...
If you need a longer session, increase the timeout at warmup time:
    ...
    blacksmith testbox warmup blacksmith-testbox.yml --idle-timeout 60

## With options

    blacksmith testbox warmup blacksmith-testbox.yml --ref main
    blacksmith testbox warmup blacksmith-testbox.yml --idle-timeout 60
